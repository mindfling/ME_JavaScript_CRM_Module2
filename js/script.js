...
const createElem = (tag, attr = {}, text) => {
  ...
  console.log('elem: ', elem); // todo debug
  return elem;
};
...
const createRow = (rowNumber, {
  ...
  const cellNumb = createElem('td', {
    className: 'table__cell',
  },
  rowNumber);

  const cellID = createElem('td', {
    'className': 'table__cell table__cell_left table__cell_name',
  },
  title);
  cellID.prepend(createElem('span', {
  ...
  // todo all cells
  row.append(cellNumb, cellID, cellCategory, cellUnits, cellCount, cellPrice, cellSum);
  return row;
};
